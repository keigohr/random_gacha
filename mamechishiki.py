...
mamechishiki_list = [
    ...
    "小規模かつランダムなあみだくじは偏りがある。縦線6本・横線10本（完全ランダム）の場合、左端→左端：37.4%に対し、左端→右端：1.3%しかない。",
    "n種類のコンプガチャを2nlogn回(=期待回数の2倍)引いた場合、コンプリート出来ない確率は1/n以下。",
    "多人数でのじゃんけんはオススメしない。10人だと、1回当たりのあいこの確率が94.8%となる。",
    # 6 ~ 7 : 降水確率についての豆知識
    "降水確率の予報と実際の結果を比較すると、予報が50%以下だった場合の実際の降水確率は、予報よりおよそ10%低い。",
    "気象庁は毎日夕方に「明日の降水の有無」の予報を出している。的中率は徐々に上がり、東京地方では現在85%超である。"
]
...
def mame_set():
    ...
    return mamechishiki_list[random_index], mamechishiki_source[random_index]

# layout に挿入できる形式でのリストを返す
def get_mamechishiki_list():
    sentence, source = mame_set()
    return [
